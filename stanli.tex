...
\begingroup
\hspace{7mm}\lstinline[emph={support}]|\support{7}{insertion point}[rotation];|

\leftskip=14mm Type $7$ is a floating bearing, identical to type $2$ but drawn with "rollers".

\leftskip=14mm\begin{minipage}[c]{0.3\linewidth}
	\begin{tikzpicture}[framed]
	\draw[orange,dashed](0,0) -- (7mm,0mm)(0,0)--(30:	7mm);
	\draw[orange,->] (.7,0) arc (0:30:7mm);
	\node[orange] (x) at (1.1,.2) {$30^\circ$};
	\point{a}{0}{0};
	\support{7}{a}[30];
	\end{tikzpicture}
\end{minipage}
...
\begingroup
\hspace{7mm}\lstinline[emph={support}]|\support{8}{insertion point}[rotation];|

\leftskip=14mm Type $8$ is identical to type $4$ but drawn with "rollers".
...
\begingroup
\hspace{7mm}\lstinline[emph={hinge}]|\hinge{6}{insertion point}[rotation];|

\leftskip=14mm Type $6$ is an alternative representation of type $3$.
